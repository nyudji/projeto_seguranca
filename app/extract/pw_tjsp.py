...
async def consultar_processos(cpf_cnpj: str, nome_completo: str) -> None:
    ...
    try:
        async with async_playwright() as p:
            ...
            async def consultar(tipo, campo, valor, msg_sem_processo, grau) -> bool:
                await page.goto(URL)
            ...
            # 1º grau
            try:
                if await consultar("DOCPARTE", "#campo_DOCPARTE", cpf_cnpj, "Não existem informações", 1):
                    logging.info(f'Processos encontrados para o CPF/CNPJ: {cpf_cnpj}')
                elif await consultar("NMPARTE", "#campo_NMPARTE", nome_completo, "Não existem informações disponíveis para os parâmetros informados.", 1):
            ...
            try:
                await page.goto(URL)
                if await consultar("DOCPARTE", "#campo_DOCPARTE", cpf_cnpj, "Não existem informações", 2):
                    logging.info(f'Processos encontrados para o CPF/CNPJ: {cpf_cnpj} - 2º grau')
                elif await consultar("NMPARTE", "#campo_NMPARTE", nome_completo, "Não existem informações disponíveis para os parâmetros informados.", 2):
